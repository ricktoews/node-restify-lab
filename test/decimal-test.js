...
describe('Decimal calculator tests', function() {
  it('should return the period of 142857 for 1/7, with 6 repeating digits.', function() {
    var result = getDecimal(7, 1);
    expect(result.period).to.equal('142857');
    expect(result.stats).to.eql({ nonRepeating: 0, repeating: 6 });
  });

  it('should return the period of 076923 for 1/13, with 6 repeating digits.', function() {
    var result = getDecimal(13, 1);
    expect(result.period).to.equal('076923');
    expect(result.stats).to.eql({ nonRepeating: 0, repeating: 6 });
  });

  it('should return the period of 047619 for 1/21, with 6 repeating digits', function() {
    var result = getDecimal(21, 1);
    expect(result.period).to.equal('047619');
    expect(result.stats).to.eql({ nonRepeating: 0, repeating: 6 });
  });

  it('should return the period of 0588235294117647 for 1/17, with 16 repeating digits', function() {
  ...
  it('should return the period of 125 for 1/8, with 3 non-repeating digits.', function() {
    var result = getDecimal(8, 1);
    expect(result.period).to.equal('125');
    expect(result.stats).to.eql({ nonRepeating: 3, repeating: 0 });
  });

  it('should return the period of 0625 for 1/16, with 4 non-repeating digits.', function() {
    var result = getDecimal(16, 1);
    expect(result.period).to.equal('0625');
    expect(result.stats).to.eql({ nonRepeating: 4, repeating: 0 });
  });

  it('should return the period of 083 for 1/12, with 2 non-repeating and 1 repeating digits.', function() {
